...
def _configure_pyproject_python(project_dir: Path, package_name: str) -> None:
    ...
    if "ruff" in config.get("tool", {}):
        ...
        # Still ensure _version.py is excluded — it's generated by hatch-vcs
        # and won't conform to ruff standards.
        version_file = f"src/{package_name}/_version.py"
        ruff_config = config["tool"]["ruff"]
        for key in ("exclude", "extend-exclude"):
            existing = list(ruff_config.get(key, []))
            if version_file in existing:
                break
        else:
            # Not found in either key — append to extend-exclude
            existing = list(ruff_config.get("extend-exclude", []))
            existing.append(version_file)
            ruff_config["extend-exclude"] = existing
            click.echo(f"  Added {version_file} to [tool.ruff] extend-exclude")
    else:
...
def _run_adopt_python(project_dir: Path, no_publish: bool) -> None:
    ...
    _run_command(
        ["uv", "add", "--group", "dev", "pytest>=9.0.0", "pytest-cov>=6.0", "ruff>=0.14.0", "pre-commit"],
        cwd=project_dir,
    )
